# Алгоритмы и структуры данных

## Archive Pattern

- The archive associates a course project on data structures and algorithms with
  Матьяш Валерий Анатольевич.
- The related personal project uses a catalog-style information system and requires
  discussion of data structures, algorithms, tests, and design choices.
- Source: the supplied archive. Detailed course requirements come from the current task and methodology.

## Required Agent Behavior

- Explain why each data structure and algorithm was selected.
- Include complexity, boundary cases, and tests in the report.
- Do not assume the archive project is the current required variant.
